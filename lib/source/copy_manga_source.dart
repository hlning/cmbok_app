...
/// 拷贝漫画源（代码源）：适配现有 [ComicApi]，实现 [MangaSource] 契约。
///
/// 内部 wrap ComicApi，最小改动不重写 copymanga 逻辑（解密/双路径/请求头均复用 ComicApi）。
/// 这是阶段1唯一的源，后续阶段追加 ConfigDrivenSource（配置源）等。
class CopyMangaSource extends MangaSource {
  final ComicApi _api;

  CopyMangaSource({ComicApi? api}) : _api = api ?? ComicApi();

  @override
  String get id => 'copymanga';
  ...
  @override
  String? get referer => 'https://2025copy.com/';
}
